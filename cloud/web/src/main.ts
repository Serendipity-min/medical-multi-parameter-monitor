...
// 这里只消费冻结的 WebSocket Snapshot v1；页面不直接理解 CANopen 或 MQTT。
type Signal = {
  node_id: string;
  stream: string;
  timestamp: number;
...
// 断连或切换网关时清空当前读数、波形及序列锚点，防止旧设备数据残留。
function clearLive(reason: string) {
  text('mode', reason);
  text('source-banner', '等待数据源');
  text('source-note', '暂无有效数据');
  for (const name of channels) setScalar(name, null, false);
  for (const name of Object.keys(buffers)) {
    buffers[name] = [];
    text(`quality-${name}`, 'STALE');
    document.getElementById(`empty-${name}`)!.hidden = false;
  }
  rrTrend = [];
  text('rr-trend-status', '等待 RR 数据');
  text('bp-updated', '—');
  text('updated', '最后采集时间：—');
  text('replay-status', '尚未收到补传');
  text('event-status', '无事件');
  for (const key of Object.keys(seen)) delete seen[key];
  for (const key of Object.keys(anchors)) delete anchors[key];
  setNode('gateway', 'Gateway', 'OFFLINE');
  setNode('node-a', 'Node-A', 'OFFLINE');
  setNode('node-b', 'Node-B', 'OFFLINE');
}

// streams 只承载当前值；replay 单独展示，不能回灌波形或当前 RR 趋势。
function render(data: Snapshot) {
  text('gateway-id', data.gateway_id);
  setNode('gateway', 'Gateway', data.gateway_state);
  setNode('node-a', 'Node-A', data.nodes['NODE-A']);
  setNode('node-b', 'Node-B', data.nodes['NODE-B']);
  const sources = [...new Set(data.streams.map((signal) => signal.source))];
  text('mode', `${data.gateway_state} · ${sources.join(' / ') || '等待数据'}`);
  text(
    'source-banner',
    sources.includes('MOCK') ? 'MOCK 模拟数据' : sources.join(' / ') || '等待数据源',
  );
  text(
    'source-note',
    sources.includes('MOCK') ? '含合成数据，用于系统联调' : '来源和有效性见各参数标签',
  );
  const latest = Math.max(0, ...data.streams.map((signal) => signal.timestamp));
  text(
    'updated',
    `最后采集时间：${latest ? new Date(latest).toLocaleString('zh-CN', { hour12: false }) : '—'}`,
  );
  ...
  for (const name of [...channels, ...Object.keys(buffers)]) {
    ...
    if (name in buffers) {
      ...
      if (seen[name] !== key) {
        ...
        buffers[name].push(
          ...signal.samples.map((y, i) => ({
            ...
            y,
            gap: i === 0 && !!previous && start - previous.t > 100,
          })),
        );
      ...
      if (name === 'NIBP')
        text(
          'bp-updated',
          valid ? new Date(signal!.timestamp).toLocaleTimeString('zh-CN', { hour12: false }) : '—',
        );
      if (name === 'RR') {
        if (!valid || typeof signal?.value !== 'number') {
          rrTrend = [];
          text('rr-trend-status', '无有效 RR 数据');
...
// 每次连接只在首帧提交令牌；回调核对 socket 身份，避免旧连接覆盖新会话。
function connect() {
  clearTimeout(retryTimer);
  intentional = false;
  const ws = new WebSocket(
    `${location.protocol === 'https:' ? 'wss:' : 'ws:'}//${location.host}/medical-monitor/ws/v1/monitor`,
  );
  socket = ws;
  ...
  ws.onclose = (event) => {
    ...
    if (event.code === 1008) {
      token = '';
      text('access-message', '访问令牌或来源校验失败，请重新输入。');
      if (!accessDialog.open) accessDialog.showModal();
      return;
    }
    if (!intentional && token) {
      const delay = Math.min(1000 * 2 ** retry++, 10000);
      text('connection', `${delay / 1000} 秒后自动重连`);
...
document.querySelectorAll<HTMLButtonElement>('[data-window]').forEach((button) => {
  button.onclick = () => {
    ...
    document.querySelectorAll('.window-start').forEach((label) => {
      label.textContent = `−${windowMs / 1000} 秒`;
    });
  };
});
document.getElementById('fullscreen')!.onclick = async () => {
  try {
    ...
    text('connection', '浏览器未允许全屏，可使用 F11');
  }
};
document.addEventListener('fullscreenchange', () => {
  document.querySelector('#fullscreen span')!.textContent = document.fullscreenElement
    ? '退出全屏'
    : '全屏显示';
});
setInterval(() => {
...
// 按设备像素比设置位图，但用 CSS 像素绘图；每帧恢复变换，避免缩放累计。
...
// 动画基于单调时钟推进窗口；采样间隙只断线，不插值成未经采集的连续信号。
function draw() {
  const now = performance.now();
  for (const name of Object.keys(buffers)) {
    ...
    for (const point of buffers[name]) {
      const x = width * (1 - (now - point.t) / windowMs),
        y = height * (name === 'RESP' ? 0.5 - point.y * 0.4 : 0.73 - point.y * 0.58);
  ...
  ctx.strokeStyle = colors.RESP;
  ctx.fillStyle = colors.RESP;
  ctx.lineWidth = 1.7;
  ctx.beginPath();
