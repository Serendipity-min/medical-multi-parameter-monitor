...
def payload(session, seq, stream, value=None, source='MOCK', validity='VALID', timestamp=None):
    return {'timestamp': timestamp or int(time.time()*1000), 'seq': seq, 'session_id': session,
            'validity': validity, 'source': source, 'synthetic': True, 'value': value, 'unit': UNITS[stream]}
...
def wave(stream, seq):
    ...
    for i in range(rate//5):
        ...
        phase = t*1.2 % 1
        if stream == 'ECG':
            y = .1*math.sin(2*math.pi*phase)+math.exp(-((phase-.25)/.022)**2)-.22*math.exp(-((phase-.30)/.028)**2)
        elif stream == 'PPG':
    ...
    while time.monotonic() < end:
        ...
        try:
            while time.monotonic() < end:
                ...
                messages = []
                if seq % 10 == 0:
                    messages.append((base+'/status', payload(session, seq, 'GATEWAY_STATUS', 'ONLINE'), 1, True))
                    for node in CHANNELS:
                        online = control.get('node_a_online' if node == 'NODE-A' else 'node_b_online', True)
                        messages.append((base+'/'+node+'/status', payload(session, seq, 'NODE_STATUS', 'ONLINE' if online else 'OFFLINE', validity='VALID' if online else 'OFFLINE'), 1, True))
                ...
                for node, streams in CHANNELS.items():
                    for stream in sorted(streams):
                        if stream not in WAVES and seq % 5:
                            continue
                        body = payload(session, seq, stream, values.get(stream), source, control.get('validity', 'VALID'), stamp)
                        if stream in WAVES:
                            body.update(wave(stream, seq))
                        kind = 'replay' if source == 'REPLAY' else 'telemetry'
                        messages.append((f'{base}/{node}/{kind}/{stream.lower()}', body, 1 if source == 'REPLAY' or stream not in WAVES else 0, False))
                if control.get('event') and seq % 10 == 0:
                    messages.append((base+'/NODE-B/event', payload(session, seq, 'FAULT', 'SYNTHETIC_TEST_EVENT'), 1, False))
                acknowledgements = []
                for topic, body, qos, retain in messages:
                    info = client.publish(topic, json.dumps(body, separators=(',', ':')), qos=qos, retain=retain)
                    if qos:
            ...
            if not abnormal and client.is_connected():
                offline = payload(session, 2**53-1, 'GATEWAY_STATUS', 'OFFLINE', validity='OFFLINE')
