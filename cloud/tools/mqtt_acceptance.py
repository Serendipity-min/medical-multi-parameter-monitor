...
def main():
    ...
    def connect(config, will=None):
        ...
        state={}
        def on_connect(c,u,f,reason,p):state['code']=reason.value
        client.on_connect=on_connect
        client.on_message=lambda c,u,m:received.put((m.topic,bytes(m.payload),m.retain))
        if will:client.will_set(will[0],will[1],qos=1,retain=True)
        client.connect_async(config['host'],config.get('port',8883),10)
        client.loop_start();clients.append(client)
    ...
    def find(received,topic,body,retained=None,seconds=5):
        ...
        while time.monotonic()<end:
            try:t,b,r=received.get(timeout=.1)
            except queue.Empty:continue
            if t==topic and b==body and (retained is None or r==retained):return True
        return False
    try:
        observer,events,code=connect(reader);assert code==0
        subscribe(observer,'mpm/v1/+/+/#');subscribe(observer,'mpm/v1/+/status')
        passed('TLS certificate verified and backend subscribed')
        bad={**publisher,'password':'intentionally-invalid-test-password'}
        client,_,code=connect(bad);assert code!=0;client.disconnect();client.loop_stop()
        passed('Wrong password rejected')
        client,_,code=connect({**publisher,'username':None,'password':None})
        assert code!=0;client.disconnect();client.loop_stop()
        ...
        try:
            with socket.create_connection((publisher['host'],8883),timeout=5) as raw:
                with context.wrap_socket(raw,server_hostname=publisher['host']):pass
        except ssl.SSLCertVerificationError:passed('Untrusted certificate rejected')
        else:raise AssertionError('CA verification missing')
        with socket.create_connection((publisher['host'],8883),timeout=5) as raw:
            raw.sendall(b'\x10\x0c\x00\x04MQTT\x04\x02\x00\x0a\x00\x00')
            try:reply=raw.recv(4)
        ...
        client,inbound,code=connect(publisher);assert code==0
        ...
        body={'timestamp':int(time.time()*1000),'seq':2**53-1,'session_id':session,'validity':'OFFLINE','source':'MOCK','synthetic':True,'value':'OFFLINE','unit':'state'}
        will=json.dumps(body).encode()
        victim,_,code=connect(publisher,(status,will));assert code==0
        body.update(seq=0,value='ONLINE',validity='VALID')
        online=json.dumps(body).encode()
        publish(victim,status,online,True)
        fresh,retained,code=connect(reader);assert code==0
        ...
        args.output.write_text(json.dumps({'checks':checks,'passed':len(checks)},indent=2),encoding='utf-8')
