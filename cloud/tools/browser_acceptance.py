...
def main():
    ...
    parser.add_argument(
        '--restart-ssh-alias', help='显式指定时，测试本项目 Backend 重启与 Nginx reload'
    )
    ...
    def cli(*parts, script=None):
        ...
        with tempfile.TemporaryFile(mode='w+', encoding='utf-8') as output:
            result = subprocess.run(
                [executable, '--session', 'medical-monitor-mqtt-v07', *parts],
                input=script,
                stdout=output,
                stderr=output,
                text=True,
    ...
    # 测试控制通过 SSH 别名转交模拟器；实际目标和访问凭据由外部配置提供。
    def control(**values):
        ...
        if args.control_ssh_alias:
            ...
            code = (
                ...
                + repr(json.dumps(values))
                + ")\nt.chmod(0o644)\nt.replace(p)"
            )
            result = subprocess.run(
                ['ssh', args.control_ssh_alias, 'sudo python3 -'],
                input=code,
                text=True,
                capture_output=True,
                timeout=20,
    ...
    def login():
        ...
        cli(
            'eval',
            '--stdin',
            script='document.querySelector("#view-token").value='
    ...
    try:
        ...
        assert evaluate(
            'document.querySelectorAll("[data-module]").length === 4 && document.querySelector("#value-TEMP").textContent === "36.6" && document.querySelector("#source-banner").textContent.includes("MOCK")'
        )
        assert evaluate('!document.querySelector("#access-dialog").open')
        passed('Four modules plus TEMP and explicit MOCK source')
        wait(
            'Array.from(document.querySelector("#wave-ECG").getContext("2d").getImageData(0,0,300,100).data).some((v,i,a) => i%4===1 && v>150 && v>a[i-1]*1.2)'
        )
        # 核验 RR 图确实绘制了接收到的 RR，而不是仅创建空 Canvas。
        wait(
            '(() => { const c=document.querySelector("#trend-RR"); return Array.from(c.getContext("2d").getImageData(0,0,c.width,c.height).data).some((v,i,a)=>i%4===0 && v>160 && a[i+1]>130 && a[i+2]<150); })()'
        )
        ...
        assert evaluate(
            'document.querySelector("#pressure-sys").textContent === "118" && document.querySelector("#pressure-dia").textContent === "76"'
        )
        passed('NIBP SYS/DIA pair renders without an invented pressure waveform')
        cli('click', '[data-window="16"]')
        assert evaluate(
            'document.querySelector("[data-window=\\"16\\"]").getAttribute("aria-pressed") === "true" && document.querySelector(".window-start").textContent.includes("16")'
        )
        ...
        wait(
            'document.querySelector("#state-HR").textContent.includes("INVALID") && document.querySelector("#value-HR").textContent === "—"'
        )
        assert evaluate(
            'document.querySelector("#value-RR").textContent === "—" && document.querySelector("#rr-trend-status").textContent.includes("无有效")'
        )
        passed('Invalid UI hides numeric values')
        control(node_a_online=False)
        wait(
            'document.querySelector("#value-SpO2").textContent === "—" && document.querySelector("#value-HR").textContent === "72"'
        )
        passed('Node-A offline UI isolation')
        control(node_b_online=False)
        wait(
            'document.querySelector("#value-HR").textContent === "—" && document.querySelector("#value-SpO2").textContent === "98"'
        )
        ...
        wait(
            'document.querySelector("#value-HR").textContent === "—" && document.querySelector("#gateway").textContent.includes("ONLINE")'
        )
        ...
        wait(
            'document.querySelector("#gateway").textContent.includes("OFFLINE") && document.querySelector("#value-HR").textContent === "—"'
        )
        control()
        wait('document.querySelector("#value-HR").textContent === "72"')
        ...
        if args.restart_ssh_alias:
            result = subprocess.run(
                [
                    ...
                    '-o',
                    'BatchMode=yes',
                    args.restart_ssh_alias,
                    'sudo -n systemctl restart medical-monitor',
                ],
                capture_output=True,
                timeout=30,
            )
            assert result.returncode == 0
            wait(
                'document.querySelector("#value-HR").textContent === "72" && document.querySelector("#connection").textContent.includes("已连接")',
                timeout=20,
            )
        ...
        wait(
            'document.querySelector("#gateway-id").textContent === "GW-C-001" && document.querySelector("#value-HR").textContent === "—"'
        )
        ...
        if args.restart_ssh_alias:
            result = subprocess.run(
                ['ssh', args.restart_ssh_alias, 'sudo systemctl stop medical-monitor-mqtt'],
                capture_output=True,
                timeout=30,
            )
            assert result.returncode == 0
            ...
            result = subprocess.run(
                ['ssh', args.restart_ssh_alias, 'sudo systemctl start medical-monitor-mqtt'],
                capture_output=True,
                timeout=30,
            )
            assert result.returncode == 0
            wait(
                'document.querySelector("#gateway").textContent.includes("ONLINE") && document.querySelector("#value-HR").textContent === "72"',
                timeout=25,
            )
        ...
        (args.output / 'browser-acceptance.json').write_text(
