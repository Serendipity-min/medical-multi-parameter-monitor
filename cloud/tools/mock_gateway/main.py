...
# 保留第一阶段旧 WSS 夹具以便追溯；生产设备入口已转为 MQTT，不能把此载荷当真采样。
def make_frame(seq: int, session_id: str, control: dict | None = None) -> dict:
    ...
    return {
        ...
        'nodes': {
            'NODE-A': {
                'online': control.get('node_a_online', True),
                'signals': {
                    'PPG': wave('PPG', 50),
                    'SpO2': scalar(98.0),
                    'PR': scalar(72.0),
            ...
            'NODE-B': {
                'online': control.get('node_b_online', True),
                'signals': {
                    'ECG': wave('ECG', 250),
                    'RESP': wave('RESP', 50),
                    'HR': scalar(72.0),
...
def read_control(path: Path | None) -> dict:
    ...
    try:
        ...
        if (
            ...
            or data.get('mode', 'LIVE') not in {'LIVE', 'REPLAY'}
            or data.get('quality', 'VALID') not in {'VALID', 'INVALID', 'STALE'}
        ):
            raise ValueError
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--config', type=Path, help='外部 JSON 配置，包含 device_url 和 device_token'
    )
    ...
    url = config.get(
        'device_url', os.environ.get('MONITOR_DEVICE_URL', 'ws://127.0.0.1:18765/device/v1/ingest')
    )
