...
def main():
    ...
    expiry=subprocess.run(['openssl','x509','-checkend','86400','-noout','-in',str(source)],capture_output=True)
    if expiry.returncode:
        raise RuntimeError('Source certificate expires too soon')
    for original,name in [(source,'server-chain.pem'),(Path(config['private_key']),'server-key.pem')]:
        temporary=root/'tls'/(name+'.next')
        shutil.copyfile(original,temporary)
        temporary.chmod(0o640)
        shutil.chown(temporary,group='mosquitto')
        temporary.replace(root/'tls'/name)
    subprocess.run(['systemctl','kill','--kill-who=main','-s','HUP','medical-monitor-mqtt'],check=True,capture_output=True)
