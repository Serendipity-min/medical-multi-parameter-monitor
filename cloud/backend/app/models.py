...
# 禁止未知字段、NaN 和隐式类型转换；任何入口都必须满足同一套单位/来源/有效性约束。
    ...
    def validate_stream(self):
        ...
        if self.stream in {'GATEWAY_STATUS', 'NODE_STATUS'}:
            ...
            if (
                ...
                or self.source == 'REPLAY'
                or self.value not in ('ONLINE', 'OFFLINE')
            ):
        ...
        if self.stream in WAVES:
            if (
                ...
                or (self.validity == 'VALID' and not self.samples)
            ):
