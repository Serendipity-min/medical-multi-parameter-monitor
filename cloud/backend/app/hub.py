...
class Hub:
    ...
    def ingest(self, message: Telemetry) -> bool:
        if message.gateway_id not in self.gateways or message.timestamp > self.clock() * 1000 + 5000:
        ...
        if message.source == 'REPLAY':
            ...
            if previous:
                ...
                is_will = message.stream == 'GATEWAY_STATUS' and message.value == 'OFFLINE' and message.session_id == old.session_id
                # 同一进程自动重连后 Will 的保留序号不能永久挡住新 ONLINE。
                recovering = message.stream == 'GATEWAY_STATUS' and old.value == 'OFFLINE' and message.value == 'ONLINE' and message.timestamp > old.timestamp
                if not (is_will or recovering) and (message.timestamp < old.timestamp or (message.session_id == old.session_id and message.seq <= old.seq)):
                    return False
    ...
    def _state(self, gateway: str, node: str) -> str:
        ...
        if message.value == 'OFFLINE' or message.validity == 'OFFLINE':
            return 'OFFLINE'
        if message.validity != 'VALID' or self.monotonic() - received > 15 or self.clock()*1000 - message.timestamp > 15000:
            return 'STALE'
        return 'ONLINE'
    ...
    def snapshot(self, gateway: str) -> dict:
        ...
        for node, names in CHANNELS.items():
            ...
            for name in sorted(names):
                ...
                elif self.monotonic()-received > limit or self.clock()*1000-message.timestamp > limit*1000:
                    data['validity'] = 'STALE'
                if data['validity'] != 'VALID':
                    data['value'], data['samples'] = None, []
                streams.append(data)
        replay, event = self.replays.get(gateway), self.events.get(gateway)
        return {'type': 'snapshot', 'schema_version': 1, 'gateway_id': gateway,
                'gateways': self.gateways, 'gateway_state': gateway_state,
                'broker_connected': self.broker_connected, 'nodes': nodes,
                'server_time': int(self.clock()*1000), 'streams': streams,
