...
def test_preflight_remains_enforced_under_optimized_python(tmp_path, failure):
    ...
    code = (
        'import sys;from pathlib import Path;sys.path.insert(0,' + repr(str(ROOT / 'deploy')) + ');'
...
def test_safe_bundle_passes_without_creating_target(tmp_path):
    ...
    module('upgrade_mqtt').validate_preflight(
        bundle,
...
def test_broker_errors_preserve_category_and_code_without_raw_text():
    ...
    assert classify(
        'OpenSSL Error[0]: error:0A000086:SSL routines::certificate verify failed at example.invalid'
    )[0:2] == ('tls_error', 'error')
    ...
    assert classify('Warning: unrecognized text password=test-only')[0:2] == (
        'broker_warning',
        'warning',
    )
