...
def test_live_and_replayed_emcy_use_same_browser_schema_without_current_alarm():
    ...
    hub.ingest(
        message('NODE-B/event', value='EMCY-0000', unit='code', source='LIVE', synthetic=False)
    )
    hub.ingest(
        message(
            'NODE-B/replay/fault',
...
def test_wave_and_replay_topic_validation():
    wave = message(
        'NODE-B/telemetry/ecg', value=None, unit='mV', samples=[0.0, 1.0], sample_rate=250
    )
    assert wave.stream == 'ECG'
    with pytest.raises(ValueError):
        message('NODE-B/telemetry/ecg', value=None, unit='mV', samples=[], sample_rate=250)
    with pytest.raises(ValueError):
        message('NODE-A/telemetry/hr')
    with pytest.raises(ValueError):
        message('NODE-B/telemetry/fault', value='x', unit='code')


def test_future_gateway_and_bounded_subscriber():
    hub, _ = ready()
    with pytest.raises(ValueError):
        hub.ingest(message(timestamp=1006000))
...
def test_browser_auth_and_no_device_ingest(monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        with pytest.raises(WebSocketDisconnect):
            with client.websocket_connect('/device/v1/ingest'):
                pass
        with client.websocket_connect(
            '/ws/v1/monitor', headers={'origin': 'http://testserver'}
        ) as ws:
            ws.send_json(
                {'token': 'unit-test-view-token-not-production-123', 'gateway_id': 'GW-C-001'}
            )
