...
> **历史版本提示**：本 v1.0 保留第一阶段编写时的流程和优先级，不再用作 P4 当前维护入口。现行统一手册为[跨阶段维护 v1.1](../../90_维护/多参数监护仪_P端云网关维护手册与工作流程_v1.1.md)。
...
当前 PR 完全关闭 Broker 日志，建议整改为：

```text
connection_messages false
不记录 Payload
保留 error / warning / 必要 notice
输出到 journald/syslog
```
...
```bash
sudo journalctl -u medical-monitor-mqtt --since "30 min ago"
```

不得把真实密码、Payload 或患者数据复制到 Git。
...
当前 Backend 单 worker、内存状态。重启不会保留完整历史。
