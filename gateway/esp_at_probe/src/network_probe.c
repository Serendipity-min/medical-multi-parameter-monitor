...
static const char *queries[] = {
  "AT", "AT+GMR", "AT+CWMODE?", "AT+SYSFLASH?", "AT+SYSRAM?", "AT+SYSSTORE?"
};
    ...
    if (c == '\n') {
      ...
      if (!discard && strcmp(line, "probe") == 0) {
        ...
        for (uint32_t i = 5; line[i]; ++i) {
          if ((uint8_t)line[i] < 32U || line[i] == '"' || line[i] == '\\') safe = false;
        }
        if (safe) {
          char command[64];
        ...
        } else console_write("INVALID_SSID\r\n");
