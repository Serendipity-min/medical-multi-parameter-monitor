...
# 此工具会通过串口断网并可重启项目 Broker；仅在获准的硬件验收窗口显式运行。
async def main(args):
    ...
    try:
        async with connect(config['browser_url'], origin=config['origin']) as ws:
            ...
            async def wait(predicate, timeout=90):
                ...
                while time.monotonic() < deadline:
                    ...
                    if not transitions or transitions[-1]['state'] != state:
                        transitions.append(
                            {'seconds': round(time.monotonic() - started, 2), 'state': state}
                        )
            ...
            def valid(snapshot):
                ...
                return (
                    snapshot['gateway_state'] == 'ONLINE'
                    and len(streams) == 9
                    and all(
                        item['validity'] == 'VALID'
                        and item['source'] == 'MOCK'
                        and item['synthetic']
                        for item in streams.values()
                    )
                    and streams['HR']['value'] == 73
                    and streams['RR']['value'] == 16
            ...
            await wait(
                lambda s: valid(s)
                and next(i for i in s['streams'] if i['stream'] == 'HR')['seq'] >= first['seq'] + 5
            )
            checks.append(
                'STM32 Paho MQTT 3.1.1 publishes nine synthetic streams through verified TLS'
            )
            ...
            if args.ssh_alias:
                ...
                try:
                    result = subprocess.run(
                        ['ssh', args.ssh_alias, 'sudo systemctl stop medical-monitor-mqtt'],
                        capture_output=True,
                        timeout=30,
                    )
                    ...
                    result = subprocess.run(
                        ['ssh', args.ssh_alias, 'sudo systemctl start medical-monitor-mqtt'],
                        capture_output=True,
                        timeout=30,
                    )
                    assert result.returncode == 0
                await wait(valid, 100)
                checks.append(
                    'STM32 reconnects after Broker restart and republishes retained state'
                )
