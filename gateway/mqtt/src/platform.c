...
{
  while (*message) { while (!(USART1->SR & USART_SR_TXE)) { } USART1->DR = (unsigned char)*message++; }
}
...
void delay_ms(uint32_t ms) { Timer timer; TimerCountdownMS(&timer, ms); while (!TimerIsExpired(&timer)) { } }
