...
资源：text 17876、data 84、bss 19980 字节。采用有界 UART 中断环形缓冲，不是最终 RTOS/DMA 架构。
...
发布九路 MOCK（含 ECG250Hz、PPG50Hz、RESP50Hz 的一秒批次）；真实 Node 和正式 RR 算法尚未接入。
canopen_adapter.h 仅预留数据类型，不冻结 OD/PDO。
