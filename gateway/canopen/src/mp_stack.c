...
static int init_node(MpStack *s, unsigned number)
{
    ...
    n->port = (MpCanPort){number, true, NULL};
    n->tick = 0;
    n->boot++;
...
int mp_stack_restart(MpStack *s, unsigned node)
{
    if (node < 1 || node > 3)
        return -1;
    CO_delete(s->nodes[node - 1].co);
    s->nodes[node - 1].co = NULL;
    s->resets[node - 1]++;
...
{
    if (node >= 1 && node <= 3)
        s->nodes[node - 1].port.enabled = enabled;
}
