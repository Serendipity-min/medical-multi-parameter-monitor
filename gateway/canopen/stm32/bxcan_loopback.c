...
/* FIFO0 每轮最多取三帧，仅接受标准数据帧；读完释放邮箱以便下一帧进入。 */
void mp_bus_hardware_poll(void)
{
    ...
    while ((CAN1->RF0R & 3) && budget--)
    {
        ...
        f.ident = (uint16_t)(id >> 21);
        f.dlc = CAN1->sFIFOMailBox[0].RDTR & 15;
        uint32_t lo = CAN1->sFIFOMailBox[0].RDLR, hi = CAN1->sFIFOMailBox[0].RDHR;
        memcpy(f.data, &lo, 4);
        memcpy(f.data + 4, &hi, 4);
        CAN1->RF0R = CAN_RF0R_RFOM0;
        if (!(id & (CAN_RI0R_IDE | CAN_RI0R_RTR)))
            mp_bus_receive(&f);
    }
}

/* 使用邮箱0非阻塞提交；返回 0 表示暂忙，驱动队首不能因此被提前删除。 */
int mp_bus_transmit(MpCanFrame *frame)
{
    ...
    if (!(CAN1->TSR & CAN_TSR_TME0))
        return 0;
    uint32_t lo, hi;
    memcpy(&lo, frame->data, 4);
    memcpy(&hi, frame->data + 4, 4);
    CAN1->sTxMailBox[0].TDTR = frame->dlc;
    CAN1->sTxMailBox[0].TDLR = lo;
    CAN1->sTxMailBox[0].TDHR = hi;
    CAN1->sTxMailBox[0].TIR = ((uint32_t)frame->ident << 21) | CAN_TI0R_TXRQ;
    return 1;
}
